 ...
 * norm, |g| is sufficently smaller: i.e., |g| &lt; eps*|g0| the second checks if
...
public class QNMinimizer implements Minimizer<DifferentiableFunction> {
  ...
  public int its = 0; // the number of iterations
  ...
  private double ftol = 1e-4; // Linesearch parameters
  ...
  public boolean bracketed = false; // used for linesearch
  QNInfo presetInfo = null;
  ...
  public enum eState {
    TERMINATE_MAXEVALS, TERMINATE_RELATIVENORM, TERMINATE_GRADNORM, TERMINATE_AVERAGEIMPROVE, CONTINUE
  }
  ...
  private static class NumericallyZeroGradient extends Throwable {
    public NumericallyZeroGradient(String s) {
      super(s);
    }
  }
  ...
  public class Record {
    ...
    private double gNormInit = Double.MIN_VALUE; // This makes is so if for
                                                  // some reason you try and
                                                  // divide by the inital norm
    // before its been initialized you dont get a NAN but you will also never
    ...
    public void add(double val, double[] grad, double[] x, int fevals) {
  ...
  public class QNInfo {
      ...
      case DIAGONAL:
        ...
        if (d != null) {
          ...
          if (x.length != d.length) {
            System.err
                .println(" Vector of incorrect size passed to applyInitialHessian in QNInfo class ");
            System.exit(1);
          }
          // Scale element-wise
     ...
     * the quasi newtion optimization routine.
     *
     ...
     * secant pairs s = x - prevoiusX and y = grad - previousGrad.
     *
  ...
  private void computeDir(double[] dir, double[] fg, QNInfo qn)
  ...
  private static double[] plusAndConstMult(double[] a, double[] b, double c,
	...
	@Override
	public double[] minimize(DifferentiableFunction function, double functionTolerance, double[] initial) {
	...
	public double[] minimize(DifferentiableFunction function, double functionTolerance, double[] initial,
			int maxIterations) {
		return minimize(function, functionTolerance, initial, maxIterations, null);
	}

  public double[] minimize(DifferentiableFunction dfunction, double functionTolerance,
    ...
    if (outputToFile) {
      try {
        ...
        System.err.println("Caught IOException outputing QN data to file: "
            + e.getMessage());
        System.exit(1);
      }
    }

    Record rec = new Record(quiet, functionTolerance, outFile);
    ...
    do {
      ...
      try {
        ...
        case MINPACK:
          ...
          break;
        default:
          sayln("Invalid linesearch option for QNMinimizer . ");
          System.exit(1);
          break;
        ...
        sayln();
        sayln("QNMinimizer aborted due to maxiumum number of function evaluations");
        sayln(m.toString());
        ...
        sayln("** program by checking the QNMinimizer.wasSuccesful() method.        ");
        break;
  ...
  private double evaluateFunction(DifferentiableFunction dfunc, double[] x, double[] grad) {
   ...
   * conditions.
   */
  private double[] lineSearchBacktrack(DifferentiableFunction func, double[] dir, double[] x,
  ...
  private double[] lineSearchMinPack(DifferentiableFunction dfunc, double[] dir,
    ...
    do {
      ...
      if (stage1 && newPt[f] <= bestPt[f] && newPt[f] > fTest) {
        ...
        infoc = getStep(x, dir, newX, f0, g0, newPt, bestPt, endPt, stpMin,
            stpMax);
   ...
   *
   * It was then interpreted from matlab implementation supplied by Andrew
   * Bradley. Modification have been made for this particular application.
   *
   * This function is used to find a new safe guarded step to be used for
   * linesearch procedures. Comments in the function were also taken from the
