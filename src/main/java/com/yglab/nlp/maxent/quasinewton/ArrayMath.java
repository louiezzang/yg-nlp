...
public class ArrayMath {
	...
	public static double[] pairwiseMultiply(double[] a, double[] b) {
		if (a.length != b.length) {
			throw new RuntimeException("Can't pairwise multiple different lengths: a.length=" + a.length + " b.length="
					+ b.length);
		}
		double[] result = new double[a.length];
	...
	/**
	 * Returns the sum of the portion of an array of numbers between <code>fromIndex</code>, inclusive, and
	 * <code>toIndex</code>, exclusive. Returns 0 if <code>fromIndex</code> &gt;= <code>toIndex</code>.
	 */
	...
	/**
	 * Returns the largest value in a vector of doubles. Any values which are NaN or infinite are ignored. If the vector
	...
	/**
	 * Returns the log of the sum of an array of numbers, which are themselves input in log form. This is all natural
	 * logarithms. Reasonable care is taken to do this as efficiently as possible (under the assumption that the numbers
	 * might differ greatly in magnitude), with high accuracy, and without numerical overflow.
	 * 
	 * @param logInputs
	...
	/**
	 * Returns the log of the portion between <code>fromIndex</code>, inclusive, and <code>toIndex</code>, exclusive, of
	 * an array of numbers, which are themselves input in log form. This is all natural logarithms. Reasonable care is
	 * taken to do this as efficiently as possible (under the assumption that the numbers might differ greatly in
	 * magnitude), with high accuracy, and without numerical overflow. Throws an {@link IllegalArgumentException} if
	 * <code>logInputs</code> is of length zero. Otherwise, returns Double.NEGATIVE_INFINITY if <code>fromIndex</code>
	 * &gt;= <code>toIndex</code>.
	 * 
	 * @param logInputs
	 *          An array of numbers [log(x1), ..., log(xn)]
	...
	/**
	 * Makes the values in this array sum to 1.0. Does it in place. If the total is 0.0 or NaN, throws an
	 * RuntimeException.
	 */
	...
	/**
	 * Standardize values in this array, i.e., subtract the mean and divide by the standard deviation. If standard
	 * deviation is 0.0, throws an RuntimeException.
	 */
	...
	/**
	 * Makes the values in this array sum to 1.0. Does it in place. If the total is 0.0, throws a RuntimeException. If the
	 * total is Double.NEGATIVE_INFINITY, then it replaces the array with a normalized uniform distribution. CDM: This
	 * last bit is weird! Do we really want that?
	 */
	...
	/**
	 * Returns the Jensen Shannon divergence (information radius) between a and b, defined as the average of the kl
	 * divergences from a to b and from b to a.
	 */
	...
	/**
	 * Returns the mean of a vector of doubles. Any values which are NaN or infinite are ignored. If the vector is empty,
	 * 0.0 is returned.
	 */
	...
	/**
	 * Returns the standard deviation of a vector of doubles. Any values which are NaN or infinite are ignored. If the
	 * vector contains fewer than two values, 1.0 is returned.
	 */
	...
	/**
	 * Direct computation of Pearson product-moment correlation coefficient. Note that if x and y are involved in several
	 * computations of pearsonCorrelation, it is perhaps more advisable to first standardize x and y, then compute
	 * innerProduct(x,y)/(x.length-1).
	 */
	...
	/**
	 * Computes the significance level by approximate randomization, using a default value of 1000 iterations. See
	 * documentation for other version of method.
	 */
	...
	/**
	 * Takes a pair of arrays, A and B, which represent corresponding outcomes of a pair of random variables: say, results
	 * for two different classifiers on a sequence of inputs. Returns the estimated probability that the difference
	 * between the means of A and B is not significant, that is, the significance level. This is computed by
	 * "approximate randomization". The test statistic is the absolute difference between the means of the two arrays. A
	 * randomized test statistic is computed the same way after initially randomizing the arrays by swapping each pair of
	 * elements with 50% probability. For the given number of iterations, we generate a randomized test statistic and
	 * compare it to the actual test statistic. The return value is the proportion of iterations in which a randomized
	 * test statistic was found to exceed the actual test statistic.
	 * 
	 * @param A
	 *          Outcome of one r.v.
	 * @param B
